...
public class AskEmailSender {

    public boolean sendEmail(AskLogic askLogic, String username, String subject, String body) {
        ...
        Properties props = this.getProperties(resourceBundleEmail);
        String from = resourceBundleEmail.getString("mail.smtp.user");
        String to = resourceBundleEmail.getString("to");
        String cc = null;
        String bcc = null;
        File[] attachments = null;
        boolean toStdOut = true;// for debug true;

        try {
            if (this.sendMail(props, from, to, cc, bcc, subject, attachments, body, toStdOut) == 0) {
                return true;
            }
        } catch (Exception e) {
            try {
                throw new AskException(askLogic, "Wrapping exception for send mail.", username, e);
    ...
    private Properties getProperties(ResourceBundle resourceBundleEmail) {
     ...
     */
    public int sendMail(final Properties props, String from, String to, String cc, String bcc, String subject,
